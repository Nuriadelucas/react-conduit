...
import { login, register, getCurrentUser, updateUser } from './auth.service';
...
describe('getCurrentUser', () => {
  it('gets the current user from /user', async () => {
    server.use(
...
describe('updateUser', () => {
  ...
  it('puts a new password to /user', async () => {
    server.use(
      http.put(`${BASE}/user`, () => HttpResponse.json({ user: mockUser }))
    );
    const { data } = await updateUser({ password: 'newpassword' });
    expect(data.user.username).toBe('testuser');
  });
});
